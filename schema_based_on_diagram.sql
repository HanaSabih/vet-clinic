...
TABLE patients{    id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    name VARCHAR(255),
    date_of_birth DATE,
};

TABLE mediacal_histories{    id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    admitted_at TIMESTAMP,
    patient_id INT,
    status VARCHAR(255),
    FOREIGN KEY (patient_id) REFERENCES patients(id)
...
TABLE treatments{    id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    type VARCHAR(255),
    name VARCHAR(255),
};

TABLE invoices{    id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    total_amount DECIMAL,
    generated_at TIMESTAMP,
    payed_at TIMESTAMP,
    mediacal_history_id INT,
...
TABLE invoice_items{id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
unit_price DECIMAL,
quantity INT,
total_price DECIMAL,
invoice_id INT,
...
TABLE mediacal_histories_treatments{id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
mediacal_histories_id INT,
treatments_id INT,
FOREIGN KEY (mediacal_histories_id) REFERENCES mediacal_histories(id),
FOREIGN KEY (treatments_id) REFERENCES treatments(id)
